        ...
        <template #content>
            <div class="mt-4">
                <div class="flex justify-between mb-4">
                    <h3 class="text-2xl font-bold">Ajout du pays</h3>
                ...
                <form>
                    <div class="space-y-2">
                        <Label for="raison_social" value="Entrez le pays" />
                        <InputIconWrapper>
                            <template #icon>
                                <UserIcon aria-hidden="true" class="w-5 h-5" />
                            </template>
                            <Input withIcon id="raison_social" type="text" placeholder="saisissez le pays"
...
const soumettre = () => {
    form.post(route("country.store"), {
        onSuccess: (page) => {
            //emit("recharge");
            useSwalSuccess("Pays ajoutée avec succès!");
            setIsOpened(false);
            form.reset();
        },
        onError: (errors) => {
