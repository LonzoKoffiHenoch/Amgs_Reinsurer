    ...
    <MyModal :show="props.show" @close="setIsOpened(false)" :closeable="true" :max-width="'xl'" :max-height="'h-[60%]'">
        <template #content>
            <div class="mt-4">
                ...
                <form>
                    <div class="space-y-2">
                        <Label for="raison_social" value="Entrez la country" />
                        <InputIconWrapper>
                            <template #icon>
                                <UserIcon aria-hidden="true" class="w-5 h-5" />
                            </template>
                            <Input withIcon id="raison_social" type="text" placeholder="Saisissez la country"
                                class="block w-full" v-model="form.label" required autofocus />
                        </InputIconWrapper>
                    </div>

                    <Button class="justify-center gap-2 text-center mt-4" @click.prevent="soumettre(props.country.id)">
...
const props = defineProps(["show", "country"])
...
onBeforeUpdate(() => {
    form.label = props.country.label;
    // console.log(props);

});
const soumettre = (id) => {
    form.post(route("country.update", {
        pays: id
    }), {
        onSuccess: (page) => {
