<template>
    <div>
        ...
        <CreateCountry @toggle="(value) => (show = value)" :show="show" />
        <EditCountry @toggle="(value) => (showEdit = value)" :show="showEdit" :country="banque" />
                    ...
                    <div class="border-b border-gray-200 shadow">
                        <table class="divide-gray-300 table-fixed w-full" id="dataTable">
                            ...
                            <tbody class="bg-white divide-y divide-gray-300">
                                <template v-for="(country, indexes) in countries" :key="indexes">
                                    <tr class="
                                            text-center
                                            whitespace-nowrap
                                            text-[10px]
                                        ...
                                        <td class="px-6 py-4">
                                            <a @click.prevent="EditcountryShow(country.id)"
...
const EditcountryShow = (id) => {
    axios
        .get(route("country.edit", {
            "pays": id
        }))
        .then((response) => {
            // console.log(response.data);
            banque.value = response.data.countries;
            showEdit.value = true;
            // console.log(banque);
...
const deletecountry = (id) => {
    Inertia.delete(route("country.delete", { pays: id }), {
        onSuccess: (response) => {
...
const deleteConfirmation = (country) => {
    // console.log(country);
    const message = `Vous êtes sur le point de supprimer le pays "${country.label}", voulez-vous continuer?`;
